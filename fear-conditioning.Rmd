...
animal_info <- read.csv('Data_concat.csv')
...
ggplot(data=hn_group, aes(x=Time, y=len, color=Genotype)) +
  geom_line(stat="identity", width=0.5) +
  facet_grid(.~Treatment) + 
  labs(y="Average Percent Freezing Time", x='Component') +
  geom_errorbar(aes(ymin=len-se, ymax=len+se), width=0.2, size=0.5)+
...
day1HN_violin<-all_day1 %>% 
  ...
  filter(Age_group == '12 months') %>% 
  select(Genotype, Pct.Total.Time.Freezing, Treatment) 
...
ggplot(data = day1HN_violin, aes(factor(Genotype), Pct.Total.Time.Freezing)) +
  geom_violin() +
...
day2_violin<-all_day2 %>% 
  ...
  filter(!is.na(Age_group)) %>% 
  filter(Age_group == '12 months') %>% 
  select(Genotype, comp, Treatment, Pct.Total.Time.Freezing)

ggplot(data = day2_violin, aes(factor(Genotype), Pct.Total.Time.Freezing)) +
  geom_violin() +
  geom_boxplot(width = 0.1) +
