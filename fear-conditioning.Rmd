...
#write.csv(all_day0, '/Users/AnnaMacFarlane/Desktop/FC_day0_data.csv')
...
day1_violin<-all_day1 %>% 
  ...
  filter(!is.na(Sex)) %>% 
  filter(!is.na(Age_group)) %>% 
  select(Pct.Total.Time.Freezing, Age_group, Sex, Coll_Geno) 
  
par(mfrow=c(1,1))
jpeg("day1_coll_violin_facetGeno_sexAge.jpeg", units="in", width=7, height=5, res=300) 
...
day2_violin_coll<-all_day2 %>% 
  ...
  filter(!is.na(Sex)) %>% 
  filter(!is.na(Coll_Geno)) %>% 
  select(comp, Age_group, Sex, Pct.Total.Time.Freezing, Coll_Geno)
